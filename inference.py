...
class QuebecFrenchGenerator:
    ...
    def generate(
        ...
        if respond is True:
            chat = [
                    {"role": "system", "content": "Répondez"},
                    {"role": "user", "content": prompt}
                ]
            inputs = self.tokenizer.apply_chat_template(chat, tokenize=True, add_generation_prompt=True, return_tensors="pt")
        else:
            # Encode prompt
...
def main():
    ...
    qfg = QuebecFrenchGenerator(args.model_path, not args.base_model, args.device)
