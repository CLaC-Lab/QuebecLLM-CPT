...
class ChatProcessor:
    ...
    def __init__(self, tokenizer, config: DataConfig, replay=False):
        self.tokenizer = tokenizer
    ...
    def load_text_data(self, file_path: str, replay_path=None, replay_percentage=0.01) -> List[str]:
        """Load and lightly clean text data"""
        logger.info(f"Loading instruction examples from {file_path}")
        ...
        if replay_path is not None:
            logger.info(f"Loading replay data from {replay_path}")
            ...
            replay_texts = replay_texts[:len(len(replay_texts)*replay_percentage)]
            logger.info(f"Using {len(replay_texts)}")

            logger.info(f"Merging with instructions")
            texts = random.shuffle([*texts, *replay_texts])
